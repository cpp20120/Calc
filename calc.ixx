...
class Parser {
 public:
  static std::expected<std::vector<Token<T>>, ArithmeticError> parse(
          ...
          if (!expect_operand &&
            ...
            while (num_end < expression.size() &&
                   (std::isdigit(expression[num_end]) ||
                    expression[num_end] == '.' || expression[num_end] == ',')) {
              if (std::isdigit(expression[num_end])) found_digit = true;
              num_end++;
            }
            ...
            auto num = parse_number<T>(num_str);
...
class Evaluator {
 public:
  static std::expected<T, ArithmeticError> evaluate(
